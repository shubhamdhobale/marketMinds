...
<div className="bg-white shadow-2xl mx-auto rounded-lg mb-10 md:mt-32 mt-8 w-full max-w-lg p-6 sm:p-10">
  ...
  <h2 className="text-2xl sm:text-3xl font-extrabold mb-2 text-center tracking-wider">Add New Trade Entry</h2>
  ...
  <div style={{ position: "relative" }} className="flex gap-4 items-center ">
    ...
    <div className="flex flex-wrap gap-2 md:gap-4">
      ...
      <div className="flex gap-4 md:gap-4 flex-row items-center">
        ...
        <input name="entryTime" id="entryTime" type="time" onChange={(e) => handleChange(e.target.name, e.target.value)} required className="border p-2 rounded-md w-full sm:w-auto"/>
        <label htmlFor="exitTime" className="text-gray-600 text-sm">Exit Time:</label>
        <input name="exitTime" id="exitTime" type="time" onChange={(e) => handleChange(e.target.name, e.target.value)} required className="border p-2 rounded-md w-full sm:w-auto"/>
      </div>
    </div>
    
    <div className="flex flex-row gap-2">
      <Input name="entryPrice" type="number" placeholder="Entry Price" onChange={(e) => handleChange(e.target.name, e.target.value)} required className="w-full sm:w-auto"/>
      <Input name="exitPrice" type="number" placeholder="Exit Price" onChange={(e) => handleChange(e.target.name, e.target.value)} required className="w-full sm:w-auto"/>
    </div>

    <div className="flex flex-row gap-2">
      <Input name="stopLoss" type="number" placeholder="Stop-Loss Price" onChange={(e) => handleChange(e.target.name, e.target.value)} className="w-full sm:w-auto"/>
      <Input name="quantity" type="number" placeholder="Quantity" onChange={(e) => handleChange(e.target.name, e.target.value)} required className="w-full sm:w-auto"/>
    </div>

    <Input name="pnl" type="number" placeholder="Total PnL" value={trade.pnl} onChange={(e) => handleChange(e.target.name, e.target.value)} />
    <Input name="date" type="date" onChange={(e) => handleChange(e.target.name, e.target.value)} required className="w-full"/>
    <Input name="strategy" placeholder="Strategy Used" onChange={(e) => handleChange(e.target.name, e.target.value)} required className="w-full"/>
    <Textarea name="reason" placeholder="Reason for Trade" onChange={(e) => handleChange(e.target.name, e.target.value)} className="w-full"/>
    ...
    <div className="flex flex-row gap-4 justify-center">
      <Button type="submit" className="bg-[#4ECCA3] hover:bg-[#4ecca2e1] text-white w-full sm:w-auto cursor-pointer">Submit</Button>
      <Button type="button" className="bg-gray-400 hover:bg-gray-500 text-white w-full sm:w-auto cursor-pointer" onClick={() => navigate("/profile")}>Cancel</Button>
    </div>
  </form>
</div>
